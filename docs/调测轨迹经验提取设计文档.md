...
| models/llm_caller.py | 同步调用，支持配置超时和输出上限 | 检查 HTTP 状态及 Message；失败返回空字符串，并只记录异常类型、状态码、响应类型和响应长度 |
...
memory 容器中运行两个 Python 进程：server.py 处理网页和 API 请求，task_runner.py 负责周期任务。调测轨迹经验提取由 task_runner.py 的事件循环触发，再通过 asyncio.to_thread 放到一个工作线程中运行，避免同步的 HTTP、模型、SQL 和 ES 操作阻塞其他定时任务。

单轮任务按顺序处理轨迹，不并行调用模型。开始时先重试 SQL 中的 pending 和 failed 记录，再读取新的增量 Step。单条轨迹失败时记录 failed 并继续处理下一条；SQL 状态保存失败时终止本轮，避免继续推进增量游标。当前服务按单实例部署，不设置任务锁。

| 运行单元 | 主要职责 | 执行方式 |
| --- | --- | --- |
| server.py 进程 | 处理现有网页和 API 请求 | 与 task_runner.py 独立运行 |
| task_runner.py 进程 | 管理定时周期和异常日志 | asyncio 事件循环 |
| 调测经验工作线程 | 执行 run_pipeline_once | 每轮占用一个工作线程，等待完成后再进入下一周期 |
| 单条轨迹处理 | 查询完整轨迹、调用模型、写入经验和状态 | 按 record_key 顺序执行 |

```plantuml
@startuml
participant "task_runner.py\n事件循环" as Runner
participant "调测经验工作线程\nrun_pipeline_once" as Worker
database "GaussDB\n任务状态表" as SQL
participant "调测数据平台" as Source
participant "模型网关" as Model
database "ES 经验库" as ES

loop 每个执行周期
  Runner -> Worker : asyncio.to_thread(...)
  activate Worker

  Worker -> SQL : 查询 pending / failed
  loop 在重试数量上限内
    Worker -> Source : 查询完整轨迹
    Source --> Worker : 全部 Step
    Worker -> Model : 提取经验
    Model --> Worker : 模型结果
    Worker -> ES : 创建、覆盖或删除经验
    Worker -> SQL : 保存处理结果
  end

  Worker -> Source : 按 updateTime + id 查询增量
  Source --> Worker : 一页 Step
  Worker -> SQL : 同一事务保存待处理轨迹和新游标
  Worker -> Worker : 顺序处理本页涉及的轨迹

  alt 本轮完成
    Worker --> Runner : 返回本轮汇总
  else SQL 或整轮异常
    Worker --> Runner : 抛出异常，保留已保存状态
  end
  deactivate Worker

  Runner -> Runner : 记录日志并等待配置周期
end
@enduml
```

进程之间不共享 SQL Session。工作线程内每次状态操作使用独立短事务；经验写入和 SQL 状态保存不是同一事务，通过固定 doc_id、source_revision 和 payload_hash 在下次运行时恢复。

### 4.5 部署视图

该功能作为 memory 服务内的定时任务部署。容器启动后同时运行 server.py 和 task_runner.py；调测经验任务默认关闭，完成手动试运行后再开启。任务使用现有 KMS 解密数据库、ES 和模型调用所需的配置。

```plantuml
@startuml
node "memory 服务服务器" {
  node "单实例 memory 容器" {
    artifact "容器启动命令" as Start
    component "server.py\n网页和 API 进程" as Web
    component "task_runner.py\n定时任务进程" as Task
    component "调测经验工作线程" as Worker
    file "settings.py\n环境变量" as Config
    ...
    Config ..> Web : 读取配置
    Config ..> Task : 读取配置
    Config ..> Worker : 读取任务参数
  }
}

cloud "CloudSpider\n调测数据平台" as Source
cloud "OAuth / KMS" as KMS
cloud "模型网关" as Model
database "GaussDB :8000\nmlops / co_mlops" as SQL
database "Elasticsearch :9200\nunified_experience_index_v4" as ES

Worker --> Source : HTTPS POST\n增量和完整轨迹
Worker --> KMS : HTTPS\n解密运行凭据
Worker --> Model : HTTPS\n提取经验和质量检查
Worker --> SQL : 数据库连接\n游标与处理状态
Worker --> ES : HTTPS\n经验及向量
...
| 外部组件 | 本功能的用途 | 部署要求 |
| --- | --- | --- |
| CloudSpider | 查询增量 Step 和完整轨迹 | 配置接口地址、请求头、超时和证书校验 |
| OAuth / KMS | 解密数据库、ES 和模型凭据 | 部署环境提供所需环境变量，不在日志中输出凭据 |
| 模型网关 | 从轨迹提取结构化经验，并执行质量检查 | 配置模型名称、上下文长度、超时和重试次数 |
| GaussDB | 保存增量游标、轨迹状态和经验引用 | 使用 co_mlops schema；首次启动前创建两张状态表 |
| Elasticsearch | 保存经验、向量和版本信息 | 测试与生产使用各自的 UNIFIED_INDEX，确认 Mapping 支持新增字段 |

首次部署时，在任务关闭状态下运行脚本初始化状态表；脚本不传参数时默认建表，也可以显式使用 `--init-tables`。随后使用 `--once --max-pages 1` 进行小范围验证。正式环境保持 TLS 校验并配置可信 CA；本地缺少内网 CA 时可临时关闭来源接口的证书校验。
...
start
:提交轨迹和待判定的修复 ID;
:调用大模型，失败或空响应最多重试 3 次;
if (找到最终 JSON 数组，且 ID 和证据检查通过?) then (是)
  :有效项生成五段式经验;
  :无效项区分确认无关联和证据不足;
  :交给“写入或删除经验”;
else (否)
...
程序允许模型在最终 JSON 数组前输出分析文字，并从回复末尾识别完整数组；数组之后出现其他文字则判定失败。随后检查 ID 是否齐全、是否重复、是否属于本产品输入，以及引用的 Step 是否存在。valid=true 的五段内容必须完整；valid=false 必须说明原因，并通过 reason_code 区分 unrelated（确认无有效关联）与 insufficient_evidence（证据不足）。证据不足仅跳过当前生成，保留原经验。JSON 解析失败或漏项属于任务失败，不能当作修复无效。

标题必须包含“主语 + 失败表现”。模型只写证据支持的结论：没有文件路径就注明未提供；fix_result 有效只说明修复有效，不能据此写成整条用例通过。详细提示词见第 8 章。

### 5.6 写入或删除经验

增量接口只用于发现发生变化的 caseId + testUser。发现变化后，系统重新查询该组合的全部历史 Step，因此模型判断时看到的是包含旧数据和新数据的完整轨迹，而不是只看本次增量记录。完整轨迹按 groupId 分开处理，并计算 source_revision；轨迹内容发生变化时，范围内的有效修复点会重新提取。

每个修复点使用固定 doc_id，计算依据为数据来源、groupId 和 fixStepId。固定 doc_id 不存在时新建，存在且经验内容变化时覆盖，内容相同则跳过。失败现象用于生成经验标题、正文和检索文本，不参与 doc_id 计算，也不用于覆盖其他轨迹的经验。
...
:重新查询该组合的全部历史 Step;
:按时间排序，再按 groupId 分开;
:计算完整产品轨迹的 source_revision;

if (与上次成功处理的版本相同?) then (是)
  :整条轨迹跳过;
else (否)
  :读取原有 experience_refs;
  :找出范围内有实际修改的修复 Step;

  repeat
    :处理一个修复点;
    :按数据来源 + groupId + fixStepId\n生成固定 doc_id;

    if (fixResult 明确为 fail 或 FAIL?) then (是)
      if (存在本任务创建的同 doc_id 经验?) then (是)
        :删除该经验;
      else (否)
        :无需删除;
      endif
    else (否)
      if (fixResult 为 success 或 PASS\n且 Diff 有实际修改?) then (是)
        :结合完整历史轨迹\n提取主失败现象和经验;

        if (模型结果完整、格式正确?) then (是)
          if (valid=true?) then (是)
            :组装五段式经验并计算 payload_hash;
            if (固定 doc_id 已存在?) then (是)
              if (payload_hash 与已有内容相同?) then (是)
                :跳过写入;
              else (否)
                :归档旧版本并覆盖经验;
                :更新向量并执行质量检测;
            ...
            if (reason_code=unrelated?) then (是)
              :删除本任务创建的同 doc_id 经验;
            else (否)
              :证据不足，保留已有经验;
            endif
  ...
  repeat while (还有本次需要判断的修复点?) is (是)
endif

:保存 experience_refs 和处理结果;
:交给“记录结果与重试”;
stop
@enduml
```
...
- pending 表示已接收但未完成；failed 表示本次处理失败，两者都需重试；done 表示本条轨迹处理完成，包括没有生成经验的情况。
...
| 函数 | 作用 |
| --- | --- |
| run_pipeline_once(config) -> RunSummary | 执行一次，返回新增、更新、删除、跳过、失败计数 |
| Pipeline.process(record_key) | 恢复或处理一组历史轨迹 |
...
| TraceStateStore.discover(...) | 单个 SQL 事务接收发现结果及页尾游标 |
...
程序从模型回复中识别末尾的完整 JSON 数组，允许数组前存在分析文字，但不接受数组后的额外内容；再验证 ID 集合与 eligible_fix_ids 完全一致、引用存在、产品一致、有效条目文本完整。修复 ID 不允许由模型新增；用户、产品及来源元数据始终由原始记录生成。Markdown 以第 3.4 节固定五段标题组装。

模型请求异常或空响应立即重试，默认最多 3 次；全部失败后记录 model_request_failed。非空但格式错误、输出截断、漏项和字段错误属于提取失败，由下次任务重试，不能当作 valid=false 删除经验。仅合法完整的业务判定进入第 5.6 节同步逻辑，insufficient_evidence 只跳过生成并保留原经验；超长分批的未完成候选不得推断无效。
...
| 模型网关空响应 | 可靠性 | HTTP 成功但响应为空或不是合法外层 JSON | 空响应最多重试 3 次，全部失败记录 model_request_failed | 已有经验保持不变 |
...
| 无法追溯 | 可观测性 | 不知道哪组处理失败 | SQL 保存 job_name、trace_key 和来源修复 ID，日志记录 record_key 与每轮汇总 | 不记录整段 Prompt |
...
| verify_tls | 默认 true；本地缺少内网 CA 时可临时设为 false，正式部署应配置可信 CA |
...
| llm_timeout / llm_retry_attempts / max_output_tokens | 300 / 3 / 10240；请求异常或空响应最多重试 3 次 |
| context_limit / safety_margin | 当前 200000 / 2048；context_limit 需按模型网关实际能力核对 |
...
| memory_service/experience_manage.py | 产品 ID 精确过滤字段支持配置 |
| api/experience_api.py | 提取共用创建覆盖、删除函数，原路由继续调用并保留 API 行为 |
| project_configs/prompt_configs.py | 新增 Prompt |
| project_configs/settings.py | 新增任务配置 |
| db_operate/sql_models.py | 两张状态表 |
| db_operate/trace_experience_store.py | 状态事务、游标保存与重试记录 |
| task_runner.py | 注册等待完成的 runner |
| models/llm_caller.py | 支持超时及输出上限参数，检查模型网关响应并输出安全诊断日志 |
| test/experience/test_trace_experience_pipeline.py | 轨迹处理、恢复、模型解析与重试测试 |
| test/experience/test_trace_experience_api_compat.py | 创建覆盖、删除、归档和原 API 响应兼容测试 |

### 10.3 上线步骤

1. 使用测试环境外部接口样例核验可选值、时间、分页和空用户的处理规则。
2. 获取 UNIFIED_INDEX Mapping，确认产品和新增 metadata 字段及向量维度。
3. 以任务关闭配置部署代码；脚本不传参数时默认初始化新增 SQL 表，也可显式使用 `--init-tables`，完成后验证可读写。
4. 暂停定时入口，通过 `--once --max-pages 1` 先读取一页增量数据，执行 8—9 月范围试运行并核对生成结果。
...
| 解析健壮性 | 数组前分析文字、数组后多余文字、数组错型、漏项 | 识别末尾合法数组，异常不判失效 |
...
| 模型请求失败 | 异常、空响应、外层响应不是 JSON | 最多重试 3 次；仍失败记录 model_request_failed，保留已有经验 |
